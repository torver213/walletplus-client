...
import { withdrawFund } from '../../libs/wallet';
type IProps = {
    isOpen: boolean;
    toggleShow: () => void
}
const WithdrawalModal: React.FC<IProps> = ({isOpen,toggleShow}) =>{
    ...
    const handleSubmit = async(e: React.FormEvent) =>{
        ...
        try {
            ...
            if(isError) return
            setIsLoading(true)
            const {msg,error} = await withdrawFund({amount, pin, userId: user._id,accountNumber},walletDispatch)
            if(!error){
              setPin(0);
              setAccountNumber(0)
              setAmount(0)
    ...
    <>
      <MDBModal tabIndex='-1' show={isOpen} backdrop>
        <MDBModalDialog centered>
          <MDBModalContent>
            <MDBModalHeader>
              <MDBModalTitle>Withdraw Fund</MDBModalTitle>
